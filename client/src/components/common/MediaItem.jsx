...
const MediaItem = ({ media, mediaType }) => {
  ...
  // ⭐ FIX QUAN TRỌNG: chuẩn hóa mediaId
  const mediaRealId = media.id || media.mediaId;

  // ⭐ SAVE WATCH HISTORY ⭐
  const saveWatchHistory = () => {
    let history = JSON.parse(localStorage.getItem("watchHistory")) || [];

    if (!history.find((item) => item.id === mediaRealId)) {
      history.push({
        id: mediaRealId,
  ...
  return (
    <Link
      to={
        mediaType !== "people"
          ? routesGen.mediaDetail(mediaType, mediaRealId)
          : routesGen.person(mediaRealId)
      }
      onClick={saveWatchHistory}   // ⭐ CLICK = LƯU HISTORY
    >
      <Box
        sx={{
      ...
      >
        ...
        {mediaType !== "people" && (
          <>
            {favoriteUtils.check({ listFavorites, mediaId: mediaRealId }) && (
              <FavoriteIcon
                color="primary"
                sx={{
                  position: "absolute",
